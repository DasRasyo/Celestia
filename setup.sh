...
echo -e "\033[38;5;205m⚠️⚠️⚠️Important NOTE: You need to back up the keys folder under the .celestia-light-blockspacerace-0 folder with WinSCP or a tool with the same function. Your key and ID information is there. Be sure to back up this folder!\033[0m"
sleep 4
...
echo -e "\033[38;5;205m⚠️⚠️⚠️Important NOTE: You need to back up the keys folder under the .celestia-full-blockspacerace-0 folder with WinSCP or a tool with the same function. Your key information is there. Be sure to back up this folder!\033[0m"
sleep 4
...
[Service]
User=$USER
ExecStart=/usr/local/bin/celestia bridge start --core.ip https://rpc-blockspacerace.pops.one --core.rpc.port 26657 --core.grpc.port 9090 --keyring.accname my_celes_key --metrics.tls=false --metrics --metrics.endpoint otel.celestia.tools:4318 --p2p.network blockspacerace
Restart=on-failure
RestartSec=3
LimitNOFILE=4096
...
echo -e "\033[38;5;205m⚠️⚠️⚠️Important NOTE: You need to back up the keys folder under the .celestia-bridge-blockspacerace-0 folder with WinSCP or a tool with the same function. Your key and ID information is there. Be sure to back up this folder!\033[0m"
sleep 5
...
do
    ...
    sync_status=$(curl -s localhost:26657/status | jq '.result | .sync_info | .catching_up')
        ...
        echo "Your node is synced with the blockspace network."
            sleep 5
            echo "Your node is now synced with the blockspace network. Proceed with validator creation."
            sleep 5
            echo "Stop the script with ctrl C and edit the following command with your information to create your validator!"
            sleep 10
            echo -e "\033[38;5;205mcelestia-appd tx staking create-validator --amount=10000000utia --pubkey=$(celestia-appd tendermint show-validator) --moniker=$moniker --chain-id=blockspacerace-0 --commission-rate=0.10 --commission-max-rate=0.20 --commission-max-change-rate=0.1 --min-self-delegation=1 --identity=KeyBaseID --details=details --website=Yourwebsite --security-contact=contactdetails --from=$wallet_name --evm-address=Evm-address\033[0m"
		sleep 20

        else
       echo "Your node is not synced with the blockspace network. Waiting for sync to complete..."
           sleep 60
        fi
done
